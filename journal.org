...
- TODO nettoyer process_descriptor.
...
- J'ai commencer à regarder process_descriptor()
  - TODO: dans la structure, marquer clairement si les variables concernent l'état ou la liste 
  - TODO: certaines variables sont modifiées directement, d'autres avec des accesseurs: uniformiser 
* <2014-05-30 ven.> (Entrée par Martin)
- coté rejeu, on est pas tout à fait au Grand Saut, mais on s'approche
  - TODO: process_handle_mediate() et ses amis devraient prendre le processus
    directement, pas le pid
  - TODO: tuer proc->is_idle de partout (et la fonction mediate_idle, et l'état, tout)
  ...
  - TODO: il faut faire un enum pour le next_state. Je parle de la poignée de
    #define dans syscall_process.c
  - TODO: on peut vouloir faire un tableau de chaînes de caractères contenant le
...
- Il reste du propre à faire coté tracing. Ce n'est pas forcément bloquant, mais
  ...
  - TODO: faire la liste des syscalls existants, et vérifier qu'ils sont tous
...
- La jonction entre le rejeu et le tracing est faite par une machine à état qui
  ...
  - Il faut arrêter d'utiliser un masque binaire pour le proc->state
    - TODO: faire un boolean séparé pour dire si on rentre dans le syscall ou si
...
- Guillaume ne connaissait pas xbt_die() (ou ça marchait moins bien y'a 2 ans). 
  - TODO: remplacer tous les XBT_ERROR()+exit() en xbt_die()
