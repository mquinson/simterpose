...
int main()
{
  ...
  if ((serverSocket = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
	...
	/* printf("clilen %d\n", clilen); */
	/* socklen_t * dest = (socklen_t * ) malloc(sizeof(socklen_t));  */
	/* dest = &clilen; */
        struct sockaddr *cli_addr = (struct sockaddr *) malloc(sizeof(struct sockaddr));

        /* if ((client_socket = accept(serverSocket, cli_addr, (socklen_t *) & clilen)) < 0) { */
        /*   perror("error accept"); */
        /*   exit(1); */
        /* } */
      }
